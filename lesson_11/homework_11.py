# Створіть масив зі строками, які будуть складатися з чисел, які розділені комою.
# Наприклад:
#
# [”1,2,3,4”, ”1,2,3,4,50” ”qwerty1,2,3”]
#
# Для кожного елементу списку виведіть суму всіх чисел (створіть нову функцію для цього).
#
# Якщо є символи, що не є числами (”qwerty1,2,3” у прикладі), вам потрібно зловити виняток і
# вивести “Не можу це зробити!”
#
# Використовуйте блок try\except, щоб уникнути інших символів, окрім чисел у списку.
#
# Для цього прикладу правильний вивід буде - 10, 60, “Не можу це зробити”


def sum_numbers(numbers_list):
    try:
        return sum(map(int, numbers_list.split(",")))
    except ValueError:
        return "Не можу це зробити!"

def main(list_numbers):
    return ", ".join(str(sum_numbers(num)) for num in list_numbers)


list_numbers = ["1,2,3,4", "1,2,3,4,50", "qwerty1,2,3"]
#list_numbers = ["1,2,3,4", "1,2,3,4,50", "1,2,3"]
print(main(list_numbers))
